...
function makeSeats(room: Room): Seat[] {
  return Array.from({ length: Math.min(room.seats, room.rows * 8) }, (_, index) => {
    const row = String.fromCharCode(65 + Math.floor(index / 8));
    const number = index % 8 + 1;
    return { id: `${row}${String(number).padStart(2, "0")}`, row, number, type: "Thường", status: index === 2 || index === 17 ? "Đã đặt" : "Trống" };
  });
}
...
export default function Cinema() {
  ...
  function changeRoom(roomId: string) { setSelectedRoomId(roomId); const room = roomMap.get(roomId); if (room) setSeats(makeSeats(room)); setSelectedSeats([]); }
  function addSeat() {
...
function TicketManager({ tickets, onExport }: { tickets: Ticket[]; onExport: () => void }) { return <><Heading title="Quản lý vé" action="Xuất báo cáo" onAction={onExport} /><section className="management-panel"><div className="panel-toolbar"><label className="search-box">⌕<input placeholder="Tìm mã vé, tên khách hàng..." /></label><button className="filter-button" type="button">▣ Tất cả trạng thái</button></div><div className="table-wrap"><table><thead><tr><th>MÃ VÉ</th><th>KHÁCH HÀNG</th><th>PHIM</th><th>GHẾ</th><th>THÀNH TIỀN</th><th>TRẠNG THÁI</th></tr></thead><tbody>{tickets.map((ticket) => <tr key={ticket.id}><td><strong>{ticket.id}</strong></td><td>{ticket.customer}</td><td>{ticket.movie}</td><td>{ticket.seats}</td><td>{money(ticket.amount)}</td><td><span className={`status ${ticket.status === "Đã thanh toán" ? "status-live" : "status-upcoming"}`}><i />{ticket.status}</span></td></tr>)}</tbody></table></div></section></>; }
...
function CustomerManager({ customers, setCustomers, onExport }: { customers: LoyaltyCustomer[]; setCustomers: (customers: LoyaltyCustomer[]) => void; onExport: () => void }) {
...
function ScheduleManager({ showtimes, movieMap, roomMap, onAdd }: { showtimes: Showtime[]; movieMap: Map<string, Movie>; roomMap: Map<string, Room>; onAdd: () => void }) { return <><Heading title="Quản lý lịch chiếu" action="+ Thêm lịch chiếu" onAction={onAdd} /><section className="management-panel"><div className="panel-toolbar"><label className="search-box">⌕<input placeholder="Tìm phim, phòng, ngày..." /></label><button className="filter-button" type="button">▣ Lọc theo ngày</button><button className="filter-button" type="button">▤ Lọc theo phòng</button></div><div className="table-wrap"><table><thead><tr><th>PHIM</th><th>PHÒNG</th><th>NGÀY</th><th>GIỜ CHIẾU</th><th>GIÁ VÉ</th><th>TRẠNG THÁI</th></tr></thead><tbody>{showtimes.map((showtime) => <tr key={showtime.id}><td><strong>{movieMap.get(showtime.movieId)?.title}</strong><small className="table-sub">{showtime.id}</small></td><td>{roomMap.get(showtime.roomId)?.name}</td><td>{showtime.date}</td><td><strong>{showtime.start}</strong> - {showtime.end}</td><td>{money(showtime.price)}</td><td><span className="status status-live"><i />Đang mở bán</span></td></tr>)}</tbody></table></div></section></>; }
