...
export default function Cinema() {
  ...
  const lastSync = realtimeClock.toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
  ...
  return <div className={`cinema-app${canManage ? "" : " cinema-readonly"}`}>
    ...
    <main className={`cinema-main${isViewChanging ? " cinema-main-changing" : ""}`}><header className="cinema-topbar"><div><p className="section-label">THỨ BA, 08 THÁNG 09, 2026</p><h1>{view === "booking" ? "Đặt vé xem phim" : isAdmin ? "Xin chào, quản trị viên." : isCustomer ? "Xin chào, hội viên Lotus." : "Không gian vận hành rạp."}</h1></div><Link href="/profile" className="admin-profile"><span>{isAdmin ? "AD" : "HV"}</span><span><strong>{isAdmin ? "Admin" : isCustomer ? "Hội viên" : "Nhân viên"}</strong><small>{isCustomer ? "Điểm thưởng cá nhân" : canManage ? "Quản lý dữ liệu" : "Đặt vé và in vé"}</small></span><i>⌄</i></Link></header>
      {notice && <div className="cinema-notice">{notice}</div>}
      {view === "overview" && <><AdminSummary movies={movies} rooms={rooms} /><Overview movies={movies} rooms={rooms} showtimes={showtimes} realtimeClock={realtimeClock} lastSync={lastSync} onNavigate={nav} /></>}
...
function Overview({ movies, rooms, showtimes, realtimeClock, lastSync, onNavigate }: { movies: Movie[]; rooms: Room[]; showtimes: Showtime[]; realtimeClock: Date; lastSync: string; onNavigate: (view: View) => void }) { const currentTimeLabel = realtimeClock.toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit", second: "2-digit" }); return <><section className="cinema-stats"><div><span className="stat-icon orange">▣</span><p>Phim đang chiếu</p><strong>{movies.filter((movie) => movie.status === "Đang chiếu").length}</strong><small>Danh mục hiện tại</small></div><div><span className="stat-icon green">▤</span><p>Phòng hoạt động</p><strong>{rooms.filter((room) => room.status === "Hoạt động").length}<em>/{rooms.length}</em></strong><small>Công suất sẵn sàng</small></div><div><span className="stat-icon blue">▰</span><p>Vé đã bán hôm nay</p><strong>{todayTickets}</strong><small>Doanh thu {todayRevenue.toFixed(1).replace(".", ",")} triệu</small></div><div><span className="stat-icon violet">↗</span><p>Doanh thu hôm nay</p><strong>{todayRevenue.toFixed(1).replace(".", ",")}<em>tr</em></strong><small>236 vé · giá bình quân 80.000đ</small></div></section><section className="realtime-banner" aria-live="polite"><div><span className="realtime-dot" /> <strong>Realtime</strong> <small>{currentTimeLabel}</small></div><span>Cập nhật {lastSync}</span></section><div className="quick-grid"><button onClick={() => onNavigate("seats")} type="button"><strong>⌗</strong><span><b>Sơ đồ ghế</b><small>Chọn và quản lý trạng thái ghế</small></span>→</button><button onClick={() => onNavigate("schedules")} type="button"><strong>▥</strong><span><b>Lịch chiếu</b><small>{showtimes.length} suất chiếu đang hoạt động</small></span>→</button><button onClick={() => onNavigate("booking")} type="button"><strong>◇</strong><span><b>Đặt vé mẫu</b><small>Kiểm tra luồng chọn ghế và thanh toán</small></span>→</button></div><Revenue /></>; }
