/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/ban-ts-comment, @next/next/no-img-element */
// @ts-nocheck
...
const moviesSeed: Movie[] = [
  ...
  { id: "MV-005", title: "Lên Hương", genre: "Kinh dị", duration: 121, release: "18/09/2026", status: "Đang chiếu", poster: "https://images.unsplash.com/photo-1517604931442-7e0c8ed2963c?auto=format&fit=crop&w=160&q=80" },
  { id: "MV-006", title: "Nghĩ Hệ Sợ Nghĩ", genre: "Kinh dị", duration: 117, release: "22/09/2026", status: "Đang chiếu", poster: "https://images.unsplash.com/photo-1524985069026-dd778a71c7b4?auto=format&fit=crop&w=160&q=80" },
  { id: "MV-007", title: "Quý Tử Vượt Giàu", genre: "Hài hước", duration: 118, release: "25/09/2026", status: "Đang chiếu", poster: "https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?auto=format&fit=crop&w=160&q=80" },
  { id: "MV-008", title: "Hồ Linh Tráng Sĩ", genre: "Hành động", duration: 135, release: "27/09/2026", status: "Đang chiếu", poster: "https://images.unsplash.com/photo-1497032628192-86f99bcd76bc?auto=format&fit=crop&w=160&q=80" },
  { id: "MV-009", title: "Cô Thuật Hắc Ngài", genre: "Kinh dị", duration: 86, release: "29/09/2026", status: "Sắp chiếu", poster: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=160&q=80" },
  { id: "MV-010", title: "Mãi Nợ Một Lời Từ", genre: "Tâm lý", duration: 120, release: "03/10/2026", status: "Đang chiếu", poster: "https://images.unsplash.com/photo-1513106580091-1d82408b8cd6?auto=format&fit=crop&w=160&q=80" },
];
...
export default function Cinema() {
  ...
  useEffect(() => {
    const syncLanguage = () => {
      const nextLanguage = (window.localStorage.getItem("lotus-locale") === "en" ? "en" : "vi") as "vi" | "en";
      setLanguage(nextLanguage);
    };

    syncLanguage();
    const handleLocaleChange = (event: Event) => {
      const customEvent = event as CustomEvent<{ locale?: "vi" | "en" }>;
      const nextLanguage = customEvent.detail?.locale ?? (window.localStorage.getItem("lotus-locale") === "en" ? "en" : "vi");
      setLanguage(nextLanguage as "vi" | "en");
    };

    window.addEventListener("lotus-locale-change", handleLocaleChange);
    const interval = window.setInterval(() => {
      setRealtimeClock(new Date());
    }, 1000);

    return () => {
      window.removeEventListener("lotus-locale-change", handleLocaleChange);
      window.clearInterval(interval);
    };
  }, []);

  const lastSync = realtimeClock.toLocaleDateString(language === "en" ? "en-US" : "vi-VN", { weekday: "long", day: "2-digit", month: "2-digit", year: "numeric" });
  const topbarGreeting = isAdmin ? (language === "en" ? "Welcome, administrator." : "Xin chào, quản trị viên.") : isCustomer ? (language === "en" ? "Welcome, Lotus member." : "Xin chào, hội viên Lotus.") : (language === "en" ? "Cinema operations workspace." : "Không gian vận hành rạp.");
  ...
  useEffect(() => {
    fetch("/api/auth/me").then(async (response) => {
      ...
      const resolvedRole = result.user.role === "admin" ? "admin" : "customer";
      setUserRole(resolvedRole);
      setIsAdmin(administrator);
      setCurrentUserName(result.user.fullName || "Khách hàng");
  ...
  const preferredMovieShowtimes = selectedMovieId ? showtimes.filter((showtime) => showtime.movieId === selectedMovieId) : showtimes;
  const currentShowtime = showtimes.find((showtime) => showtime.id === selectedShowtime) || preferredMovieShowtimes[0] || showtimes[0];
  ...
  function startNewBooking() { setSelectedSeats([]); setSelectedMovieId(""); setSelectedShowtime(""); setBookingStep("showtime"); setView("booking"); setNotice(""); }
  ...
  const customerProfile = (() => {
    const matchingCustomer = customers.find((customer) => customer.email.toLowerCase() === currentUserEmail.toLowerCase());
    return matchingCustomer ? matchingCustomer : {
      name: currentUserName || "Khách hàng",
      email: currentUserEmail || "customer@lotus.com",
      points: 0,
      tier: "Đồng",
      joined: "08/09/2026"
    };
  ...
  const handleCustomerMovieSelect = (movieId: string) => {
    ...
    setView("booking");
    setNotice("Vui lòng chọn thời gian chiếu phù hợp.");
  };
  ...
  const nav = (next: View) => {
    ...
    if (!canOpenView(next)) {
      setNotice("Tài khoản khách hàng chỉ được xem đặt vé và chương trình tích điểm.");
      setView("customers");
      return;
    }
  ...
  return <div className={`cinema-app${canManage ? "" : " cinema-readonly"}`}>
    <aside className="cinema-sidebar"><Link href="/" className="cinema-logo"><span className="brand-symbol">LT</span><span>LOTUS<br /><small>CINEMA</small></span></Link><p className="sidebar-label">{isCustomer ? "HỘI VIÊN" : "QUẢN TRỊ RẠP"}</p><nav className="cinema-nav" aria-label="Điều hướng quản trị">
      <button className={`nav-item ${view === "overview" ? "active" : ""}`} onClick={() => nav("overview")} type="button">▦ <span>Tổng quan</span></button>
      {!isCustomer && <><button className={`nav-item ${view === "movies" ? "active" : ""}`} onClick={() => nav("movies")} type="button">▣ <span>Quản lý phim</span><b>{movies.length}</b></button>
      ...
      <button className={`nav-item ${view === "customers" ? "active" : ""}`} onClick={() => nav("customers")} type="button">✦ <span>{isCustomer ? "Điểm thưởng" : "Tích điểm"}</span><b>{customerProfile.points}</b></button>
      ...
      {!isCustomer && <button className={`nav-item ${view === "booking" ? "active" : ""}`} onClick={() => nav("booking")} type="button">◇ <span>Đặt vé</span></button>}
      {!isCustomer && <button className={`nav-item ${view === "revenue" ? "active" : ""}`} onClick={() => nav("revenue")} type="button">↗ <span>Doanh thu</span></button>}
      {isCustomer && <button className={`nav-item ${view === "booking" ? "active" : ""}`} onClick={() => nav("booking")} type="button">◇ <span>Đặt vé</span></button>}
    ...
    <main className={`cinema-main${isViewChanging ? " cinema-main-changing" : ""}`}><header className="cinema-topbar"><div><p className="section-label">{lastSync.toUpperCase()}</p><h1>{view === "booking" ? (language === "en" ? "Book tickets" : "Đặt vé xem phim") : topbarGreeting}</h1></div><Link href="/profile" className="admin-profile"><span>{isAdmin ? "AD" : "HV"}</span><span><strong>{isAdmin ? (language === "en" ? "Admin" : "Admin") : isCustomer ? (language === "en" ? "Member" : "Hội viên") : (language === "en" ? "Staff" : "Nhân viên")}</strong><small>{isCustomer ? (language === "en" ? "Personal points" : "Điểm thưởng cá nhân") : canManage ? (language === "en" ? "Data management" : "Quản lý dữ liệu") : (language === "en" ? "Booking and ticketing" : "Đặt vé và in vé")}</small></span><i>⌄</i></Link></header>
      ...
      {view === "booking" && <>{bookingStep !== "showtime" && <button className="filter-button booking-back-button" type="button" onClick={startNewBooking}>← Quay lại chọn phim</button>}<BookingFlow step={bookingStep} showtimes={preferredMovieShowtimes} movieMap={movieMap} roomMap={roomMap} selectedMovieId={selectedMovieId} selectedShowtime={selectedShowtime} selectedSeats={selectedSeats} seats={seats} total={total} paymentMethod={paymentMethod} onShowtime={(id) => { const show = preferredMovieShowtimes.find((item) => item.id === id); if (show) { setSelectedMovieId(show.movieId); setSelectedShowtime(id); setBookingStep("seats"); changeRoom(show.roomId); setNotice(""); } }} onToggle={toggleSeat} onPayment={setPaymentMethod} onNext={() => setBookingStep(bookingStep === "seats" ? "payment" : "seats")} onFinish={finishPayment} /></>}
...
function CustomerOverview({ movies, onNavigate, onMovieSelect }: { movies: Movie[]; onNavigate: (view: View) => void; onMovieSelect: (movieId: string) => void }) {
  ...
  return (
    <>
      <section className="customer-hot-hero">
        <div>
          <p className="section-label">PHIM HOT CỦA TUẦN</p>
          <h2>Khám phá bộ phim đáng sợ, cuốn hút và bùng nổ cảm xúc.</h2>
        </div>
        <button className="primary-button customer-hot-cta" type="button" onClick={() => onNavigate("booking")}>Đặt vé ngay</button>
      </section>
      <section className="customer-hot-grid">
        {hotMovies.map((movie) => (
          <article className="customer-hot-card" key={movie.id} onClick={() => onMovieSelect(movie.id)} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onMovieSelect(movie.id); } }}>
            ...
            <div className="customer-hot-info">
              <div className="customer-hot-labels">
...
function CustomerMembershipCard({ customer }: { customer: LoyaltyCustomer }) {
  const pointsText = customer.points.toLocaleString("vi-VN");
  return <section className="customer-membership-card">
    <div className="customer-card-header">
      <div>
        <p className="section-label">LOTUS REWARDS</p>
        <h2>Thẻ thành viên</h2>
      </div>
      <span className="tier tier-vàng">{customer.tier}</span>
    </div>
    <div className="customer-card-body">
      ...
      <div className="customer-points-summary">
        <small>Điểm tích lũy</small>
        <strong>{pointsText}</strong>
        <em>điểm</em>
      </div>
      <div className="customer-card-meta">
        <div><small>Ngày tham gia</small><strong>{customer.joined}</strong></div>
        <div><small>Chi tiêu</small><strong>{Math.max(1200000, customer.points * 1800).toLocaleString("vi-VN")}đ</strong></div>
      </div>
      <div className="customer-card-barcode">
        <span>LOTUS</span>
        <i />
      </div>
    </div>
...
function BookingFlow({ step, showtimes, movieMap, roomMap, selectedMovieId, selectedShowtime, selectedSeats, seats, total, paymentMethod, onShowtime, onToggle, onPayment, onNext, onFinish }: { step: string; showtimes: Showtime[]; movieMap: MovieMap; roomMap: RoomMap; selectedMovieId?: string; selectedShowtime: string; selectedSeats: string[]; seats: Seat[]; total: number; paymentMethod: string; onShowtime: (id: string) => void; onToggle: (seat: Seat) => void; onPayment: (method: string) => void; onNext: () => void; onFinish: () => void }) { const movieShowtimes = selectedMovieId ? showtimes.filter((item) => item.movieId === selectedMovieId) : showtimes; const show = movieShowtimes.find((item) => item.id === selectedShowtime) || movieShowtimes[0] || showtimes[0]!; const movie = movieMap.get(show.movieId); const room = roomMap.get(show.roomId); if (step === "ticket") return <Ticket movie={movie} show={show} room={room} selectedSeats={selectedSeats} total={total} />; return <section className="booking-shell"><div className="booking-steps"><span className="done">1. Suất chiếu</span><span className={step !== "showtime" ? "done" : ""}>2. Chọn ghế</span><span className={step === "payment" ? "done" : ""}>3. Thanh toán</span></div>{step === "showtime" && <div className="showtime-picker"><h2>Chọn suất chiếu</h2><p>{movie ? movie.title : "Các suất đang mở bán"} · {movieShowtimes.length ? `${movieShowtimes.length} suất có sẵn` : "Chưa có lịch chiếu"}</p>{movieShowtimes.map((item) => <button className={selectedShowtime === item.id ? "showtime-option chosen" : "showtime-option"} key={item.id} onClick={() => onShowtime(item.id)} type="button"><strong>{item.start}</strong><span>{movieMap.get(item.movieId)?.title}<small>{roomMap.get(item.roomId)?.name} · {money(item.price)}</small></span>→</button>)}</div>}{step === "seats" && <div className="booking-seat-panel"><div className="screen-wide">MÀN HÌNH</div><div className="seat-map large-seat-map">{seats.map((seat) => <button className={`seat-button seat-${seat.status.toLowerCase()} seat-${seat.type.toLowerCase()} ${selectedSeats.includes(seat.id) ? "seat-selected" : ""}`} key={seat.id} onClick={() => onToggle(seat)} type="button">{seat.id}</button>)}</div><BookingSummary movie={movie} show={show} room={room} selectedSeats={selectedSeats} total={total} action="Tiếp tục thanh toán" onAction={onNext} /></div>}{step === "payment" && <div className="payment-panel"><div><p className="section-label">XÁC NHẬN ĐẶT VÉ</p><h2>Kiểm tra thông tin</h2><dl><dt>Phim</dt><dd>{movie.title}</dd><dt>Suất chiếu</dt><dd>{show.date} · {show.start} - {show.end}</dd><dt>Phòng</dt><dd>{room.name}</dd><dt>Ghế</dt><dd>{selectedSeats.join(", ")}</dd></dl></div><BookingSummary movie={movie} show={show} room={room} selectedSeats={selectedSeats} total={total} action="Xác nhận đặt vé" onAction={onFinish} paymentMethod={paymentMethod} onPayment={onPayment} /></div>}</section>; }
function BookingSummary({ movie, show, room, selectedSeats, total, action, onAction, paymentMethod, onPayment }: { movie: Movie; show: Showtime; room: Room; selectedSeats: string[]; total: number; action: string; onAction: () => void; paymentMethod?: string; onPayment?: (value: string) => void }) { return <aside className="booking-summary"><p className="section-label">THÔNG TIN VÉ</p><h3>{movie.title}</h3><p>{show.start} - {show.end} · {room.name}</p><strong>{selectedSeats.length ? selectedSeats.join(", ") : "Chưa chọn ghế"}</strong><div className="summary-total"><span>{selectedSeats.length} x {money(show.price)}</span><b>{money(total)}</b></div>{onPayment && <><label className="payment-select">Phương thức<select value={paymentMethod} onChange={(event) => onPayment(event.target.value)}><option>Tiền mặt</option><option>Ví điện tử</option><option>Sandbox Pay</option><option>Thẻ ngân hàng</option></select></label><div className="sandbox-payment"><span className="sandbox-pill">Sandbox</span><small>{paymentMethod === "Sandbox Pay" ? "Chế độ thử nghiệm đang bật." : "Thanh toán an toàn theo chuẩn bảo mật."}</small></div></> }<button className="primary-button" disabled={!selectedSeats.length} onClick={onAction} type="button">{action} <span>→</span></button></aside>; }
