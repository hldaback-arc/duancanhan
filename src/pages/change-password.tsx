    ...
    <div className="page-shell">
      <main className="account-layout">
        <header className="account-header">
          ...
          <Link href="/profile" className="secondary-button">← Hồ sơ</Link>
        </header>
        <section className="account-intro">
          <p className="eyebrow">BẢO MẬT THÀNH VIÊN</p>
          <h1>Đổi mật khẩu.</h1>
          <p>Dùng một mật khẩu riêng tư và khó đoán để bảo vệ tài khoản Lotus Cinema.</p>
        </section>
        <nav className="account-tabs"><Link href="/profile">Thông tin cá nhân</Link><Link href="/change-password" className="active">Đổi mật khẩu</Link></nav>
        <form onSubmit={submit} className="account-form password-form">
          ...
          <p className="password-hint">Mật khẩu mới cần tối thiểu 8 ký tự.</p>
          {error && <p className="form-message error-message">{error}</p>}
          {message && <p className="form-message success-message">{message}</p>}
          <button className="primary-button account-save" type="submit" disabled={saving}>{saving ? "Đang cập nhật..." : "Cập nhật mật khẩu"} <span>→</span></button>
        </form>
      </main>
    </div>
  );
