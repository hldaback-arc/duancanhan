...
async function reset() {
  await run("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, full_name TEXT NOT NULL, email TEXT NOT NULL UNIQUE, phone TEXT NOT NULL DEFAULT '', student_id TEXT NOT NULL DEFAULT '', class_name TEXT NOT NULL DEFAULT '', role TEXT NOT NULL DEFAULT 'customer', password_hash TEXT NOT NULL, password_salt TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)");
  ...
  if (!columns.some((column) => column.name === "phone")) await run("ALTER TABLE users ADD COLUMN phone TEXT NOT NULL DEFAULT ''");
  ...
  try {
    ...
    await run("INSERT INTO users (full_name, email, phone, student_id, class_name, role, password_hash, password_salt) VALUES (?, ?, ?, ?, 'member', 'admin', ?, ?)", [admin.fullName, admin.email, admin.phone, admin.phone, hash, salt]);
    await run("COMMIT");
  } catch (error) {
    await run("ROLLBACK");
    throw error;
